...
def save(generations=None,track_scores=None):
	if generations in [1,49,99,149,199,249,299,349,399]:
		with open('./completed_runs/'+game+'/finalruns/run_'+run+'/track_scores_'+game+'_pop50gen500_230818.pkl', 'wb') as f:
...
def save_tokens_labels_words_seen(generations=None,tokens=None,labels=None):	
	...
	if generations in [1,100,200,300,400]:
		# save the scores
		#path = './completed_runs/deephome/finalruns/run_1/saved_pickles/test_word_seen_vectors_at_'+str(generations)+'_.pkl'
		path = './completed_runs/'+game+'/finalruns/run_'+run+'/saved_pickles/test_word_seen_vectors_at_'+str(generations)+'_.pkl'		
		#path2 = './completed_runs/deephome/finalruns/run_1/saved_pickles/test_word_seen_vocabs_at_'+str(generations)+'_.pkl'
		path2 = './completed_runs/'+game+'/finalruns/run_'+run+'/saved_pickles/test_word_seen_vocabs_at_'+str(generations)+'_.pkl'
...
with open('./completed_runs/'+game+'/finalruns/run_'+run+'/final_weights_'+game+'_pop50gen500_230818.pkl', 'wb') as g:
	pickle.dump(final_weights, g)

# save the words seen in the game
with open('./completed_runs/'+game+'/finalruns/run_'+run+'/words_seen_'+game+'_pop50gen500_230818.pkl', 'wb') as h:
	pickle.dump(word_seen, h)

with open('./completed_runs/'+game+'/finalruns/run_'+run+'/test_word_seen_vectors_'+game+'_pop50gen500_230818.pkl', 'wb') as i:
	pickle.dump(tokens, i)

with open('./completed_runs/'+game+'/finalruns/run_'+run+'/test_word_seen_vocab_'+game+'_pop50gen500_230818.pkl', 'wb') as k:
	pickle.dump(labels, k)
